...
from freqtrade.data.btanalysis import load_trades_from_db, load_backtest_data, load_backtest_stats, get_latest_backtest_filename
...
def do_trade_buys(pair, data, trades, rk_tags=False, alt_tag="buy", pickled_signal_candles=None):
...
def process_one(pair, stratnames, trade_dict, all_candles, ft_config, current_count, total_job_num, verbose, write_out, use_timeframe_detail, pickled_signal_candles):
...
def process_all(pairlist, stratnames, trade_dict, all_candles, ft_config, parallel=False, verbose=False, write_out=False, use_timeframe_detail=False, pickled_signal_candles=None):
    buy_reason_jobs = []
    ...
    current_count = 1
    for pair in pairlist:
        buy_reason_jobs.append((pair, stratnames, trade_dict, all_candles, ft_config, current_count, total_job_num, verbose, write_out, use_timeframe_detail, pickled_signal_candles))
        current_count += 1
...
def main():
    ...
    parser.add_argument("-d", "--data_format", nargs='?', choices=["json", "hdf5"], help="specify the jsons or hdf5 datas. Default: 'json'")
    ...
    parser.add_argument("--signal_candles_pickle", nargs='?', help="specify the path to the pickled signal candles file. Default: None")
    ...
    results = process_all(pairlist, stratnames, trade_dict, all_candles, ft_config, parallel=parallel, pickled_signal_candles=pickled_signal_candles)
    
    print_results(results, stratnames, args.outfile, args.group, args.buy_reason_list, args.sell_reason_list, args.indicator_list, args.cancels)
